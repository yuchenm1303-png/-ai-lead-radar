...
function out(row: any) {
  return {
    ...
    actor_role: row.actor_role ?? "buyer",
...
async function proxyIngest(body: unknown) {
  if (!SB_URL || !SECRET_KEY) throw new Error("Ingest service credentials are unavailable");
  ...
  try {
    ...
    if (method === "GET" && path === "/health") {
      return json({
        ...
        role: "compatibility-read-gateway",
        version: "1.0-policy-gateway",
    ...
    if (method === "POST" && path === "/api/v1/ingest/manual") {
      if (!ALLOWED.has(origin)) return json({ detail: "Write origin required" }, 403, origin);
      const body = await req.json();
      if (!Array.isArray(body?.items) || !body.items.length) return json({ detail: "items required" }, 422, origin);
      return json(await proxyIngest({ items: body.items.slice(0, 100), query_context: null }), 200, origin);
    }
    ...
    if (method === "GET" && path === "/api/v1/monitor/status") {
      ...
      return json({
        ...
        policy_version: POLICY_VERSION,
        classifier: "lead-radar-ingest",
        notification_enabled: Boolean(FEISHU_WEBHOOK_URL),
        note: "All candidate write paths converge on lead-radar-ingest. Acquisition and classification policy are separated.",
      }, 200, origin);
    }
    ...
    if (method === "POST" && path === "/api/v1/monitor/scan") {
      ...
      return json({
        ok: true,
        delegated: true,
        scan_service: "lead-radar-scan",
        note: "Use lead-radar-scan for quota-controlled provider collection.",
      }, 200, origin);
    }
    ...
    console.error(error);
    return json({ detail: "Internal API error", policy_version: POLICY_VERSION }, 500, origin);
  }
});
